...
function getMovieDetail(imdbid) {
  return fetch(`https://www.omdbapi.com/?apikey=a8b3cdf1&i=${imdbid}`)
    .then((response) => response.json())
    .then((m) => m);
}

function updeteUIDetail(m) {
  const movieDetail = showMovieDetail(m);
  const containerMovieDetail = document.querySelector(
    ".container-movie-detail"
  );
  containerMovieDetail.innerHTML = movieDetail;
}

function getMovies(keyword) {
  return fetch(`https://www.omdbapi.com/?apikey=a8b3cdf1&s=${keyword}`)
    .then((response) => response.json())
